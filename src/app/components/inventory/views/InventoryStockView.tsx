...
export function InventoryStockView({ site }: Props) {
  ...
  const criticalCount = mockStockItems.filter(i => stockHealth(i.currentStock, i.minStock, i.maxStock) === "Critical").length;
  const lowCount = mockStockItems.filter(i => stockHealth(i.currentStock, i.minStock, i.maxStock) === "Low").length;
  const totalValue = mockStockItems.reduce((s, i) => s + i.value, 0);
  ...
  return (
    <div className="p-4 lg:p-5 space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: "Total SKUs", value: mockStockItems.length, sub: "Unique materials", color: "border-l-primary" },
      ...
      <div className="bg-card border border-border rounded-2xl p-3 space-y-2.5">
        <div className="flex items-center gap-2">
          ...
          <button className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-border text-xs font-semibold hover:bg-muted/40">
