...
import { Search, ChevronDown, ChevronRight, Download, Plus, CheckCircle2, Clock, RotateCcw } from "lucide-react";
...
export function JournalEntriesView({ fy }: Props) {
  ...
  const data = useMemo(() => mockJournalEntries.filter(j => j.fy === fy), [fy]);
  ...
  const totalPosted = filtered.filter(j => j.status === "Posted").length;
  const totalAmount = filtered.reduce((s, j) => s + j.totalDebit, 0);
    ...
    <div className="p-4 lg:p-6 space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          { label: "Draft / Pending", value: filtered.filter(j => j.status === "Draft").length.toString(), color: "border-l-muted-foreground" },
        ].map(k => (
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <select value={statusF} onChange={e => setStatusF(e.target.value)} className="px-3 py-2.5 text-xs rounded-xl border border-border bg-card focus:outline-none font-medium">
        ...
        <button className="flex items-center gap-1.5 px-3 py-2.5 rounded-xl bg-primary text-white text-xs font-semibold hover:bg-primary/90">
          <Plus size={13} /> New JV
        </button>
      </div>
              ...
              >
                ...
                <div className="flex-1 grid grid-cols-2 sm:grid-cols-4 gap-3 items-center">
                  ...
                  <div className="col-span-1 sm:col-span-2">
                    ...
                    <p className="text-[10px] text-muted-foreground">{je.createdBy} · {je.lines.length} lines</p>
                  </div>
                  <div className="flex items-center gap-2 justify-end sm:justify-start">
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold border ${STATUS_COLORS[je.status]}`}>
