...
export function LedgerView({ fy }: Props) {
  const [accountId, setAccountId] = useState(mockLedgerAccounts[0].id);
  const [search, setSearch] = useState("");
  const [voucherF, setVoucherF] = useState("All");

  const account = mockLedgerAccounts.find(a => a.id === accountId) ?? mockLedgerAccounts[0];

  const filtered = account.entries.filter(e => {
    ...
    <div className="p-4 lg:p-6 space-y-5">
      ...
      <div className="bg-card border border-border rounded-2xl p-4">
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
          <div className="flex-1">
            <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block mb-1.5">Select Account</label>
            <div className="relative">
              <select
                value={accountId}
                onChange={e => setAccountId(e.target.value)}
                className="w-full px-3 py-2.5 text-sm font-semibold rounded-xl border border-border bg-background focus:outline-none focus:ring-2 focus:ring-primary/20 appearance-none pr-8"
              >
                {mockLedgerAccounts.map(a => (
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="bg-card border border-border rounded-2xl p-4 text-center">
          ...
          <p className="text-[10px] text-muted-foreground">{filtered.filter(e => e.debit > 0).length} transactions</p>
        </div>
        <div className="bg-card border border-border rounded-2xl p-4 text-center">
          ...
          <p className="text-[10px] text-muted-foreground">{filtered.filter(e => e.credit > 0).length} transactions</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <select value={voucherF} onChange={e => setVoucherF(e.target.value)} className="px-3 py-2.5 text-xs rounded-xl border border-border bg-card focus:outline-none">
