...
export function ProjectProvider({ children, tenantId }: { children: ReactNode; tenantId?: string }) {
  const { projects: storeProjects, fetchProjects, addProject: storeAddProject } = useProjectStore();
  ...
  useEffect(() => {
    if (storeProjects && storeProjects.length > 0) {
      // Map API project response models to local UI project models
      const mapped = storeProjects.map((p: any) => ({
        id: p.id,
        tenantId: p.tenantId || tenantId || "shg-001",
        name: p.name,
  ...
  const addProject = async (projectData: Omit<Project, "id" | "tenantId"> & { subdomain?: string }) => {
    ...
    try {
      await storeAddProject({
        ...
        reraNumber: projectData.rera,
        location: projectData.location,
        tenantId: projectData.subdomain || activeTenantId,
        budget: parseFloat(projectData.budget.replace(/[^0-9.]/g, "")) || 0,
        spent: parseFloat(projectData.spent.replace(/[^0-9.]/g, "")) || 0
      });
      fetchProjects();
    } catch (err) {
