...
import { useAuthStore, apiRequest } from "../../store/store";
import {
...
function OTPLoginScreen({ onNavigate, onLogin }: { onNavigate: (s: Screen) => void; onLogin?: () => void }) {
  ...
  const sendOTP = async () => {
    ...
    try {
      ...
      if (data.devBypassCode) {
        console.log(`[Dev Bypass OTP Code]: ${data.devBypassCode}`);
      }
    } catch (err: any) {
      setLoading(false);
...
function SubdomainScreen({ onNavigate, onLogin }: { onNavigate: (s: Screen) => void, onLogin?: () => void }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setError(err.message || "Invalid credentials for this workspace.");
    } finally {
      setLoading(false);
    }
  };
