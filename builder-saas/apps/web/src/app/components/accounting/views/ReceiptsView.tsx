          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/40">
                ...
                <th className="px-4 py-3 text-center text-[11px] font-bold text-muted-foreground uppercase tracking-wider">PDF</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {filtered.length === 0 && (
                <tr><td colSpan={12} className="px-4 py-12 text-center text-muted-foreground text-sm">No receipts found</td></tr>
              )}
              {filtered.map(r => (
                <tr key={r.id} className={`hover:bg-muted/30 transition-colors ${selected.has(r.id) ? "bg-primary/5" : ""}`}>
                  ...
                  <td className="px-4 py-3 text-center">
                    <button
                      onClick={async () => {
                        try {
                          const res = await fetch("http://localhost:3000/api/pdf/receipt", {
                            method: "POST",
                            headers: {
                            ...
                            body: JSON.stringify({
                              ...
                              mode: r.mode,
                              txId: r.reference || `TXN-${r.receiptNo}`
                            })
                          });
                          if (res.ok) {
                            const d = await res.json();
                            if (d.pdfUrl) {
                              window.open(`http://localhost:3000${d.pdfUrl}`, "_blank");
                            }
                          }
                        } catch (e) {
                          console.error(e);
                      ...
                      className="px-2 py-1 rounded bg-orange-500 text-white text-[10px] font-bold hover:bg-orange-600 transition-colors"
                    >
                      PDF
                    </button>
                  </td>
